...
use crate::constants::{LIQUIDITY_WEIGHT, VOLUME_WEIGHT};
...
impl SyntheticRouter {
    ...
    pub fn initialize(
        ...
        self.aggregated_prob = U64F64::from_num(0);
    ...
    pub fn update_weights(&mut self) -> Result<()> {
        let total_weight = self.child_markets.iter()
            .map(|m| {
                ...
                let liq_weight = U64F64::from_num(m.liquidity_depth) * U64F64::from_num(LIQUIDITY_WEIGHT);
                let vol_weight = U64F64::from_num(m.volume_7d) * U64F64::from_num(VOLUME_WEIGHT);
                liq_weight + vol_weight
            })
            .fold(U64F64::from_num(0), |a, b| a + b);
        
        if total_weight == U64F64::from_num(0) {
    ...
    pub fn update_aggregated_probability(&mut self) -> Result<()> {
        ...
        self.aggregated_prob = self.child_markets.iter()
            ...
            .fold(U64F64::from_num(0), |a, b| a + b) / total_weight;
