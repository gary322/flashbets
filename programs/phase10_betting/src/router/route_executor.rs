...
impl RouteExecutor {
    ...
    pub fn calculate_route(
        ...
        match router.routing_strategy {
            ...
            RoutingStrategy::BestPriceFirst => {
                ...
                if is_buy {
                    sorted_markets.sort_by(|a, b| a.1.probability.cmp(&b.1.probability));
                } else {
                    sorted_markets.sort_by(|a, b| b.1.probability.cmp(&a.1.probability));
                }
    ...
    fn calculate_leg_cost(
        ...
        let execution_price = if is_buy {
            base_price * (U64F64::one() + U64F64::from_num(slippage) / U64F64::from_num(10_000u32))
        } else {
            base_price * (U64F64::one() - U64F64::from_num(slippage) / U64F64::from_num(10_000u32))
        };
