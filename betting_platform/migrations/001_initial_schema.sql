...
CREATE TABLE positions (
    ...
    -- Indexes
    INDEX idx_positions_wallet ON positions(wallet_address),
    INDEX idx_positions_market ON positions(market_id),
    INDEX idx_positions_status ON positions(status),
    INDEX idx_positions_opened_at ON positions(opened_at DESC)
);
...
CREATE TABLE trades (
    ...
    -- Indexes
    INDEX idx_trades_wallet ON trades(wallet_address),
    INDEX idx_trades_market ON trades(market_id),
    INDEX idx_trades_executed_at ON trades(executed_at DESC)
);
...
CREATE TABLE settlements (
    ...
    -- Indexes
    INDEX idx_settlements_wallet ON settlements(wallet_address),
    INDEX idx_settlements_status ON settlements(status),
    INDEX idx_settlements_created_at ON settlements(created_at DESC)
);
...
CREATE TABLE user_wallets (
    ...
    -- Indexes
    INDEX idx_user_wallets_address ON user_wallets(wallet_address),
    INDEX idx_user_wallets_volume ON user_wallets(total_volume DESC)
);
...
CREATE TABLE liquidity_pools (
    ...
    -- Indexes
    INDEX idx_liquidity_pools_market ON liquidity_pools(market_id)
);
...
CREATE TABLE verses (
    ...
    -- Indexes
    INDEX idx_verses_category ON verses(category),
    INDEX idx_verses_relevance ON verses(relevance_score DESC)
);
...
CREATE TABLE quantum_positions (
    ...
    -- Indexes
    INDEX idx_quantum_positions_wallet ON quantum_positions(wallet_address),
    INDEX idx_quantum_positions_market ON quantum_positions(market_id)
);
...
CREATE TABLE audit_logs (
    ...
    -- Indexes
    INDEX idx_audit_logs_wallet ON audit_logs(wallet_address),
