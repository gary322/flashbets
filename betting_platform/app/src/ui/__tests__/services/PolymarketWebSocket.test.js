...
describe('PolymarketWebSocket', () => {
    ...
    beforeEach(() => {
        jest.useFakeTimers();
        wsManager = new PolymarketWebSocket_1.PolymarketWebSocket({
    ...
    describe('Connection Management', () => {
        it('should connect to WebSocket server', () => __awaiter(void 0, void 0, void 0, function* () {
            const connectedSpy = jest.fn();
            wsManager.on('connected', connectedSpy);
            wsManager.connect();
            // Wait for connection
            yield jest.advanceTimersByTimeAsync(20);
            expect(connectedSpy).toHaveBeenCalled();
        }));
        it('should emit disconnected event on close', () => __awaiter(void 0, void 0, void 0, function* () {
        ...
        it('should attempt reconnection on disconnect', () => __awaiter(void 0, void 0, void 0, function* () {
            const connectedSpy = jest.fn();
            wsManager.on('connected', connectedSpy);
            wsManager.connect();
            yield jest.advanceTimersByTimeAsync(20);
        ...
        it('should use exponential backoff for reconnection', () => __awaiter(void 0, void 0, void 0, function* () {
            wsManager.connect();
            yield jest.advanceTimersByTimeAsync(20);
            mockWs = wsManager.ws;
            // First disconnect - 1000ms delay
            mockWs.close();
            yield jest.advanceTimersByTimeAsync(999);
            expect(wsManager.ws).toBeNull();
            yield jest.advanceTimersByTimeAsync(1);
            expect(wsManager.ws).not.toBeNull();
            // Second disconnect - 2000ms delay
            mockWs = wsManager.ws;
            mockWs.close();
            yield jest.advanceTimersByTimeAsync(1999);
            expect(wsManager.reconnectTimer).not.toBeNull();
            yield jest.advanceTimersByTimeAsync(1);
            expect(wsManager.ws).not.toBeNull();
        }));
    });
    describe('Message Handling', () => {
        ...
        it('should handle heartbeat messages', () => {
            const heartbeatSpy = jest.spyOn(wsManager, 'resetHeartbeat');
    ...
    describe('Subscription Management', () => {
        ...
        it('should queue subscriptions when not connected', () => {
            wsManager.disconnect();
            wsManager.subscribe('market1');
            expect(wsManager.subscriptions.has('market1')).toBe(true);
        });
        it('should resubscribe to markets on reconnection', () => __awaiter(void 0, void 0, void 0, function* () {
            wsManager.subscribe('market1');
            wsManager.subscribe('market2');
            mockWs.close();
            // Wait for reconnection
            yield jest.advanceTimersByTimeAsync(1000);
            const newWs = wsManager.ws;
    ...
    describe('Heartbeat Mechanism', () => {
        ...
        it('should stop heartbeat on disconnect', () => {
            const stopHeartbeatSpy = jest.spyOn(wsManager, 'stopHeartbeat');
            mockWs.close();
            expect(stopHeartbeatSpy).toHaveBeenCalled();
            expect(wsManager.heartbeatTimer).toBeNull();
        });
    });
    ...
    describe('Error Handling', () => {
        ...
        it('should clear reconnect timer on disconnect', () => __awaiter(void 0, void 0, void 0, function* () {
            wsManager.connect();
            yield jest.advanceTimersByTimeAsync(20);
            mockWs = wsManager.ws;
            mockWs.close();
            ...
            wsManager.disconnect();
            expect(wsManager.reconnectTimer).toBeNull();
        }));
    });
});
