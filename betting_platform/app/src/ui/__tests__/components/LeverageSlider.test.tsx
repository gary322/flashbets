...
describe('LeverageSlider', () => {
  ...
  describe('Rendering', () => {
    ...
    it('should display effective leverage when different from base', () => {
      const { getByText } = render(
        <LeverageSlider {...defaultProps} effectiveLeverage={25} />
      );
  ...
  describe('Warning Messages', () => {
    it('should not show warning for safe leverage', () => {
      const { queryByText } = render(
        <LeverageSlider {...defaultProps} effectiveLeverage={40} />
      );
    ...
    it('should show moderate warning at 50x+', () => {
      const { getByText } = render(
        <LeverageSlider {...defaultProps} effectiveLeverage={75} />
      );
    ...
    it('should show high leverage warning at 100x+', () => {
      const { getByText } = render(
        <LeverageSlider {...defaultProps} effectiveLeverage={150} />
      );
    ...
    it('should show extreme warning at 300x+', () => {
      const { getByText } = render(
        <LeverageSlider {...defaultProps} effectiveLeverage={450} />
      );
    ...
    it('should not show warnings when showWarnings is false', () => {
      const { queryByText } = render(
        <LeverageSlider {...defaultProps} effectiveLeverage={150} showWarnings={false} />
      );
  ...
  describe('Slider Interaction', () => {
    it('should update value when track is clicked', () => {
      ...
      if (track) {
        ...
        fireEvent.click(track, { clientX: 100 }); // Click at middle
        expect(defaultProps.onChange).toHaveBeenCalledWith(50);
      }
    });

    it('should handle drag interaction on thumb', async () => {
      const { container } = render(
        <LeverageSlider {...defaultProps} />
      );
      ...
      if (thumb) {
        ...
        fireEvent.mouseMove(thumb, { clientX: 160 });
  ...
  describe('Visual States', () => {
    it('should change track color based on warning level', () => {
      ...
      rerender(<LeverageSlider {...defaultProps} effectiveLeverage={150} />);
      track = container.querySelector('[danger="true"]');
      expect(track).toBeInTheDocument();

      rerender(<LeverageSlider {...defaultProps} effectiveLeverage={350} />);
  ...
  describe('Liquidation Calculations', () => {
    it('should calculate liquidation buffer correctly', () => {
      ...
      testCases.forEach(({ leverage, expectedBuffer }) => {
        const { getByText } = render(
          <LeverageSlider {...defaultProps} effectiveLeverage={leverage} />
        );
  ...
  describe('Animations', () => {
    it('should animate warning message appearance', async () => {
      const { container, rerender } = render(
        <LeverageSlider {...defaultProps} effectiveLeverage={40} />
      );
  ...
  describe('Edge Cases', () => {
    it('should handle zero leverage', () => {
      const { container } = render(
        <LeverageSlider {...defaultProps} value={0} effectiveLeverage={0} />
      );
    ...
    it('should handle maximum leverage', () => {
      const { getByText } = render(
        <LeverageSlider {...defaultProps} value={100} effectiveLeverage={100} />
      );
    ...
    it('should handle undefined coverage gracefully', () => {
      const { container } = render(
        <LeverageSlider {...defaultProps} coverage={undefined as any} />
      );
