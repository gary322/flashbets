...
# Uses k6 for load testing with comprehensive scenarios
...
RESULTS_DIR="$SCRIPT_DIR/results/load_test"
API_BASE_URL="${API_BASE_URL:-http://localhost:8081}"

mkdir -p "$RESULTS_DIR"

echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}Load Testing - 1000+ Concurrent Users${NC}"
echo -e "${BLUE}========================================${NC}"
echo -e "Start Time: $(date)"
echo -e "Target URL: $API_BASE_URL"
echo ""
...
cat > "$SCRIPT_DIR/load_test.js" << 'EOF'
...
echo "This will simulate up to 1500 concurrent users over ~35 minutes"
echo ""
...
if k6 run \
    --out json="$RESULTS_DIR/raw_results.json" \
    -e BASE_URL="$API_BASE_URL" \
    "$SCRIPT_DIR/load_test.js"; then
    ...
    if [ -f "$RESULTS_DIR/summary.json" ]; then
        ...
        if command -v jq > /dev/null 2>&1; then
            echo "- Total Requests: $(jq '.metrics.http_reqs.count' "$RESULTS_DIR/summary.json")"
            echo "- Success Rate: $(jq -r '100 - (.metrics.errors.rate * 100) | tostring + "%"' "$RESULTS_DIR/summary.json")"
            echo "- Median Response Time: $(jq '.metrics.http_req_duration.med' "$RESULTS_DIR/summary.json")ms"
            echo "- 95th Percentile: $(jq '.metrics.http_req_duration["p(95)"]' "$RESULTS_DIR/summary.json")ms"
        fi
    fi
    
    echo -e "\nResults saved to:"
    echo "- Summary: $RESULTS_DIR/summary.json"
    echo "- HTML Report: $RESULTS_DIR/summary.html"
    echo "- Raw Data: $RESULTS_DIR/raw_results.json"
