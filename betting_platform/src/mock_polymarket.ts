...
export class MockPolymarketServer {
    ...
    constructor(port: number = 3001, wsPort: number = 3002) {
    ...
    startMockWebSocket(port: number) {
    ...
    simulatePriceMovements(): NodeJS.Timeout {
        return setInterval(() => {
    ...
    simulateMassResolution(count: number) {
        const unresolvedMarkets = this.markets.filter(m => !m.resolved);
...
export function startMockPolymarketServer(port: number = 3001, wsPort: number = 3002): MockPolymarketServer {
