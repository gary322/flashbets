...
use betting_platform_native::{
    ...
    keeper_coordination::{KeeperCoordinator, WorkItem},
    keeper_registration::{KeeperRegistration, SlashingEvidence},
    ...
    state::{
        ...
        IngestorState, Position, WebSocketState, WorkType, WebSocketHealth,
    },
    math::U64F64,
...
async fn test_price_update_keeper() {
    ...
    for (current_slot, expected_health) in test_cases {
        websocket_state.last_update_slot_slot = 1000;
...
fn monitor_websocket_health(state: &WebSocketState, current_slot: u64) -> WebSocketHealth {
    let slots_since_update = current_slot.saturating_sub(state.last_update_slot_slot);
