...
impl MMTTestEnv {
    async fn new() -> Self {
        ...
        // Pre-derive PDAs for account creation
        ...
        // Add token accounts with proper data
    ...
    async fn initialize_early_trader_registry(&mut self, season: u8) -> Result<(), BanksClientError> {
        ...
        let instruction = Instruction {
            ...
            data: BettingPlatformInstruction::InitializeEarlyTraderRegistry { season }.try_to_vec().unwrap(),
        };
...
async fn test_maker_journey_complete() {
    ...
    for (i, (notional, improvement)) in trades.iter().enumerate() {
        ...
        if i > 0 {
            env.advance_slots(MIN_SLOTS_BETWEEN_TRADES + 1).await;
        }
    ...
    println!("   Avg improvement: {} bp", maker.metrics.average_spread_improvement_bp);
...
async fn test_early_trader_race() {
    ...
    for (i, trader) in traders.iter().enumerate() {
        ...
        let (_maker_account, _) = Pubkey::find_program_address(
...
async fn test_multi_user_ecosystem() {
    ...
    let staker1 = env.create_user("Staker1", 1_000_000 * 10u64.pow(MMT_DECIMALS as u32)).await;
    let staker2 = env.create_user("Staker2", 500_000 * 10u64.pow(MMT_DECIMALS as u32)).await;
    let maker1 = env.create_user("Maker1", 100_000 * 10u64.pow(MMT_DECIMALS as u32)).await;
    let maker2 = env.create_user("Maker2", 50_000 * 10u64.pow(MMT_DECIMALS as u32)).await;
    ...
    for day in 1..=5 {
        ...
        // Advance time
        env.advance_slots(216_000).await; // ~1 day
...
async fn register_early_trader(
    ...
    let instruction = Instruction {
        ...
        data: BettingPlatformInstruction::RegisterEarlyTrader { season }.try_to_vec().unwrap(),
    };
...
async fn claim_maker_rewards(
    ...
    let instruction = Instruction {
        ...
        data: BettingPlatformInstruction::ClaimMakerRewards.try_to_vec().unwrap(),
    };
