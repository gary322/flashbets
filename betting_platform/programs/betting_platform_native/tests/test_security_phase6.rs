...
use betting_platform_native::{
    ...
    state::{
        accounts::{discriminators, GlobalConfigPDA, LeverageTier},
...
async fn test_type_safety() {
    ...
    use betting_platform_native::account_validation::validate_discriminator;
    ...
    let result = validate_discriminator(&data, discriminators::GLOBAL_CONFIG);
    assert!(result.is_ok(), "Valid account should pass validation");
    
    // Test invalid discriminator
    let invalid_result = validate_discriminator(&data, discriminators::VERSE_PDA);
