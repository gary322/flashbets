...
use betting_platform_native::{
    state::{Position, KeeperAccount, KeeperStatus, KeeperType},
...
async fn test_keeper_receives_5bp_reward() {
    ...
    let _keeper_account_keypair = Keypair::new();
    
    let keeper_account = KeeperAccount {
        discriminator: [0u8; 8],
        keeper_id: [0u8; 32],
        authority: keeper_keypair.pubkey(),
        mmt_stake: 10_000_000_000, // $10k stake
...
async fn test_keeper_stats_updated_after_liquidation() {
    ...
    let mut keeper_account = KeeperAccount {
        ...
        keeper_id: [0u8; 32],
        authority: keeper_keypair.pubkey(),
        mmt_stake: 10_000_000_000,
        performance_score: 9500, // 95%
        total_operations: 5, // Already done 5 liquidations
    ...
    // - last_operation_slot = current_slot
    
    let new_reward = 10_000_000; // $10 from liquidating $20k
    keeper_account.total_operations += 1;
    keeper_account.successful_operations += 1;
    keeper_account.total_rewards_earned += new_reward;
    
    assert_eq!(keeper_account.total_operations, 6);
    assert_eq!(keeper_account.successful_operations, 6);
...
async fn test_keeper_reward_calculation_edge_cases() {
    ...
    let large_liquidation = 10_000_000_000_000u64; // $10M
