...
async fn test_proposal_pda_size_520_bytes() {
    let proposal = ProposalPDA {
        ...
        total_volume: 0,
        market_data: [0u8; 256],
        metadata_uri: String::from("https://example.com/metadata"),
...
async fn test_cpi_depth_tracking() {
    ...
    assert!(tracker.can_make_cpi().unwrap());
    tracker.enter_cpi().unwrap();
    assert_eq!(tracker.get_current_depth(), 1);
    
    // Test chain operation (should allow up to depth 3)
    tracker.enter_cpi().unwrap();
    tracker.enter_cpi().unwrap();
    assert_eq!(tracker.get_current_depth(), 3);
    assert!(tracker.can_make_chain_cpi().unwrap());
    
    // Test max depth
    tracker.enter_cpi().unwrap();
    assert_eq!(tracker.get_current_depth(), 4);
    assert!(!tracker.can_make_cpi().unwrap());
    
    // Test exit
    tracker.exit_cpi().unwrap();
    assert_eq!(tracker.get_current_depth(), 3);
    assert!(tracker.can_make_cpi().unwrap());
}
...
/// Test 8: Verify Polymarket rate limiting
#[tokio::test]
...
/// Test 9: Verify MMT token distribution (10M/season, 15% rebate)
#[tokio::test]
...
async fn test_newton_raphson_convergence_error() {
    let mut solver = NewtonRaphsonSolver::new();
    let pool = create_test_pmamm_pool(3);
...
/// Test 18: Verify multi-keeper parallelism
#[tokio::test]
...
/// Test 19: Verify wash trading protection
#[tokio::test]
...
async fn test_all_requirements_implemented() {
    ...
    // 1. ProposalPDA size
    assert_eq!(std::mem::size_of::<ProposalPDA>(), 520);
    
    // 2. CU optimization
    assert!(CUOptimizer::new().target_cu_per_trade == 20_000);
    
    // 3. Batch optimization
    assert!(BatchOptimizer::new().config.target_batch_cu == 180_000);
