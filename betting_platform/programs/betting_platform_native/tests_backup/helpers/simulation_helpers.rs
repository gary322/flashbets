...
use betting_platform_native::{
    ...
    state::{
        ...
        LmsrMarketPDA, PmammPoolPDA, L2DistributionPDA, DarkPoolPDA,
        IcebergOrderPDA, TwapOrderPDA, StopOrderPDA, BootstrapPDA,
    },
...
pub async fn get_bootstrap_state(
    context: &TestContext,
) -> Result<BootstrapPDA, BettingPlatformError> {
...
fn get_pmamm_pool_pda(pool_id: u128, program_id: &Pubkey) -> Pubkey {
    Pubkey::find_program_address(
        &[b"pmamm_pool", &pool_id.to_le_bytes()],
        program_id,
    ).0
}
...
fn get_l2_distribution_pda(pool_id: u128, program_id: &Pubkey) -> Pubkey {
    Pubkey::find_program_address(
        &[b"l2_distribution", &pool_id.to_le_bytes()],
        program_id,
    ).0
}
...
fn get_resolution_pda(proposal_id: u128, program_id: &Pubkey) -> Pubkey {
    Pubkey::find_program_address(
        &[b"resolution", &proposal_id.to_le_bytes()],
        program_id,
    ).0
}
